...
RSpec.describe Api::NonRecurringItemsController, type: :controller do
  let(:good_params) { FactoryBot.attributes_for(:non_recurring_item, amount: 314) }
  let(:bad_params) { FactoryBot.attributes_for(:non_recurring_item, amount: '')}
  ...
  describe 'DELETE #destroy' do
    before(:each) do 
      @item = FactoryBot.create(:non_recurring_item, user_id: @user.id)
    end
  ...
  describe 'PUT #update' do
    before(:each) do 
      @item = FactoryBot.create(:non_recurring_item, 
                                amount: 0,
                                user_id: @user.id)
    end
    ...
    context 'with no user signed in' do
      ...
      it 'does not update the item' do
        ...
        expect(item.amount).to eq(@item.amount)
      end
    ...
    context 'with the owner signed in' do
      ...
      describe 'with invalid attributes' do
        it 'does not modify the item' do
          ...
          expect(updated_item.amount).to eq(@item.amount)
        end
